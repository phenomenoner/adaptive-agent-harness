...
SERVER_NAME = "aar-mcp"
MCP_TOOL_SURFACE_VERSION = "aar.mcp-tools.v7"
MCP_PROTOCOL_VERSIONS = (
    *reversed(MODERN_PROTOCOL_VERSIONS),
    *reversed(HANDSHAKE_PROTOCOL_VERSIONS),
)
OPERATION_SKILL_VERSION = "0.9.4"
SCHEMA_BUNDLE_DIGEST = "sha256:8d54d6099289bf74f9c7ca337a81ecde994fc4f44f7e50bf4e6b0e2a5f1c2c70"
...
REFERENCE_GRANT_CAPABILITIES = {
    item.grant_id: item.capability for item in REFERENCE_GRANTS
}
...
def _program_failure(result: WorkspaceProgramResult) -> FailureEnvelope:
    ...
    return FailureEnvelope(
        ...
        code=(
            ...
            if result.workspace_lost
            else "WORKSPACE_EXECUTION_FAILED"
        ),
        message=message,
...
def _failure(
    ...
    *,
    ...
    if isinstance(
        ...
        code = (
            "BROKER_GRANT_DENIED"
            if isinstance(error, BrokerGrantDenied)
...
def _validate_read_context(host: ReferenceHost, context: McpReadContext) -> None:
...
def _skill_digest() -> str:
    ...
    if skill_path.is_file():
        ...
        bundled = importlib.resources.files("aar").joinpath(
            "bundled", "aar-operations", "SKILL.md"
        )
...
def build_server(
    ...
    *,
    ...
    async def aar_capabilities(request: Context) -> CapabilitiesToolResult:
        manifest = await tool_surface_manifest(server)
    ...
    async def aar_reference_context(
        ...
        try:
            if budget_wall_time_ms > 60_000:
                raise BudgetDenied(
                    "reference-host wall-time budget cannot exceed 60000 ms"
                )
            grant_id = REFERENCE_GRANT_IDS.get(capability)
            if grant_id is None:
                raise GrantDenied(
                    "capability has no published reference-host grant"
                )
            ready = host.ready()
            now_ms = host.now_ms()
    ...
    async def aar_workspace_create(
        ...
        try:
            ...
            if record.state is OperationState.ACCEPTED:
                ...
                handle = host.create_workspace(
                    workspace, SessionRef(value=context.session_id)
                )
    ...
    async def aar_program_workspace_interrupt(
        ...
        try:
            ...
            return ProgramWorkspaceInterruptToolResult(
                failure=_failure(error, operation=operation)
            )
    ...
    async def aar_program_workspace_restore(
        ...
        try:
            ...
            envelope = _envelope(
                ...
                canonical_sha256(payload),
                workspace=(
                    None if expected_handle is None else _envelope_handle(expected_handle)
                ),
            )
            record, _created = host.registry.accept(
                envelope, canonical_json_bytes(payload).decode()
    ...
    async def aar_program_workspace_reconcile(
        ...
        try:
            ...
            if not any(
                grant.capability == "workspace.program.execute"
                for grant in request.grants
            ):
                raise WorkspaceBindingDenied(
                    "operation is not a programmable workspace execution"
                )
            record = host.registry.get(operation)
    ...
    async def aar_asset_export(
        ...
        try:
            ...
            bundle = host.adaptive_assets.export_bundle(
                tuple(roots), include_events=include_events
            )
    ...
    async def aar_broker_catalog(
        ...
        try:
            request = _assert_operation_binding(host, context, operation)
            return BrokerCatalogToolResult(
                catalog=host.brokers.bind(request, operation).catalog()
            )
        except Exception as error:
    ...
    async def aar_operation_cancel(
        ...
        try:
            ...
            request = _assert_operation_binding(host, context, operation)
            if any(
                grant.capability == "workspace.program.execute" for grant in request.grants
            ):
