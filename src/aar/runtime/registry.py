...
SCHEMA_VERSION_CURRENT = SCHEMA_VERSION_RECOVERY_POLICY
...
class OperationRegistry:
    ...
    def request_dispatch(
        ...
        if kind != "rlm.execute":
            raise ValueError("only rlm.execute is durable in AR-LT1")
        with self._lock, self._connection:
            row = self._connection.execute(
                "SELECT * FROM operations WHERE operation_id = ?", (operation.value,)
            ).fetchone()
    ...
    def list_recovery_candidates(self) -> tuple[OperationRecord, ...]:
        """Return operations whose running dispatch is explicitly indeterminate."""
        ...
        with self._lock:
            rows = self._connection.execute(
                ...
                WHERE o.state = ? AND o.certainty = ? AND d.state = ?
    ...
    def requeue_indeterminate(
        ...
        continuation_boundary: OperationRlmStepBoundaryV1 | None = None,
    ) -> OperationRecord:
        ...
        with self._lock, self._connection:
            ...
            if decision_value == _RECOVERY_START_SUCCESSOR and policy_binding is not None:
                ...
                prior_successors = self._connection.execute(
                    ...
                    WHERE operation_id = ? AND decision = ?
                    """,
                    (operation.value, _RECOVERY_START_SUCCESSOR),
                ).fetchone()
                ...
                existing_boundary = self._connection.execute(
                    """
                    SELECT * FROM operation_rlm_boundaries
                ...
                if existing_boundary is not None:
                    ...
                    self._connection.execute(
                        """
                        INSERT INTO operation_rlm_boundaries(
