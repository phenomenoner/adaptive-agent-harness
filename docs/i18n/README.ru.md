...
**RLM × постоянный IPython × долговечные операции × полномочия у хоста**
...
> Канонический README написан на английском. В переключателе языков традиционный китайский намеренно стоит первым.
...
**Adaptive Agent Harness вместо этого даёт им программируемую рабочую среду.** Модель может хранить состояние исследования в IPython, проверять и преобразовывать длинные входные данные с помощью Python, выполнять ограниченные рекурсивные вызовы моделей или субагентов, сохранять квитанции операций и подключаться к работе через небольшой интерфейс MCP.
...
RLM нужно место, где можно **думать вместе с данными**, а не только говорить о них. IPython хорошо подходит для этого, потому что предоставляет модели постоянную вычислительную рабочую область:
...
Каждый компонент закрывает свой тип отказа:
...
Вместе они позволяют агенту переходить между языковым рассуждением и детерминированными вычислениями, не превращая каждое промежуточное значение в текст промпта.
...
> **Python — язык оркестрации; хост остаётся границей полномочий.**
...
- 30 инструментов MCP на текущей поверхности v7;
...
Перезапустите Codex App, если в квитанции настройки указано, что конфигурация изменилась, затем вызовите `aar_capabilities` в новой задаче.
...
Проверено в замороженном кандидате:

- поддержка Python с 3.11 по 3.14;
- поверхность MCP v7 из 30 инструментов;
- аддитивная схема SQLite до v4;
- полный запуск репозитория: **199 пройдено, 1 пропуск по условию платформы**;
- чистые проверки точного wheel-пакета в Linux/WSL и нативной Windows;
...
- свидетельства переключения установленного Hermes, сохранённые в журнале проекта только для добавления.
...
Перед заявлениями о готовности к производству прочитайте [TECHNICAL-STATUS.md](../../TECHNICAL-STATUS.md), [HOST-COMPATIBILITY.md](../../HOST-COMPATIBILITY.md) и [WAL.md](../../WAL.md).
