...
**RLM × IPython แบบคงสถานะ × การดำเนินการที่คงทน × อำนาจที่โฮสต์เป็นผู้ถือครอง**
...
> README ฉบับต้นฉบับเป็นภาษาอังกฤษ ตัวสลับภาษาตั้งใจให้ภาษาจีนตัวเต็มอยู่ลำดับแรก
...
**Adaptive Agent Harness จึงมอบโต๊ะทำงานที่ตั้งโปรแกรมได้แทน** โมเดลสามารถเก็บสถานะการสืบค้นไว้ใน IPython ตรวจสอบและแปลงอินพุตยาว ๆ ด้วย Python เรียกโมเดลหรือซับเอเจนต์แบบเวียนซ้ำภายใต้ขอบเขต บันทึกใบเสร็จ (receipt) ของการดำเนินการ และกลับมาเชื่อมต่องานผ่านพื้นผิว MCP ขนาดเล็กได้
...
RLM ต้องมีที่ให้คิด **ไปพร้อมกับข้อมูล** ไม่ใช่แค่พูดถึงข้อมูล IPython เหมาะกับงานนี้เพราะมอบ workspace คำนวณที่คงอยู่ให้โมเดล:
...
แต่ละส่วนครอบคลุมโหมดความล้มเหลวที่แตกต่างกัน:
...
เมื่อทำงานร่วมกัน ทั้งหมดนี้ช่วยให้เอเจนต์สลับระหว่างการให้เหตุผลด้วยภาษาและการคำนวณแบบกำหนดแน่นอนได้ โดยไม่ต้องเปลี่ยนค่าระหว่างทางทุกค่าให้เป็นข้อความในพรอมป์
...
> **Python คือภาษาสำหรับการประสานงาน; โฮสต์ยังคงเป็นขอบเขตอำนาจ**
...
อัลฟาสาธารณะปัจจุบัน: **`0.3.0a0`**

ตรวจสอบแล้วใน candidate ที่ freeze ไว้:

- รองรับ Python 3.11 ถึง 3.14;
- พื้นผิว MCP v7 จำนวน 30 เครื่องมือ;
- schema SQLite แบบเพิ่มต่อเนื่องจนถึง v4;
- การรันรีโพซิทอรีทั้งหมด: **ผ่าน 199 รายการ ข้าม 1 รายการตามเงื่อนไขแพลตฟอร์ม**;
- การตรวจสอบ exact-wheel ที่สะอาดบน Linux/WSL และ Windows แบบ native;
...
- หลักฐานการ cutover ของ Hermes ที่ติดตั้งไว้ เก็บอยู่ใน WAL ของโครงการแบบเพิ่มอย่างเดียว
...
อ่าน [TECHNICAL-STATUS.md](../../TECHNICAL-STATUS.md), [HOST-COMPATIBILITY.md](../../HOST-COMPATIBILITY.md) และ [WAL.md](../../WAL.md) ก่อนกล่าวอ้างความพร้อมใช้งานจริง
