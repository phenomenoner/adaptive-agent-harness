...
**RLM × IPython مستمر × عمليات دائمة × سلطة يملكها المضيف**
...
> ملف README الأساسي هو النسخة الإنجليزية. يضع مُبدّل اللغات اللغة الصينية التقليدية أولًا عمدًا.
...
**يمنحهم Adaptive Agent Harness بيئة عمل قابلة للبرمجة بدلًا من ذلك.** يستطيع النموذج الاحتفاظ بحالة التحقيق في IPython، وفحص المدخلات الطويلة وتحويلها باستخدام Python، وإجراء استدعاءات محدودة ومتكررة للنموذج أو للوكلاء الفرعيين، وحفظ إيصالات العمليات، وإعادة الاتصال بالعمل عبر سطح MCP صغير.
...
تحتاج RLMs إلى مكان للتفكير **بالبيانات**، لا لمجرد الحديث عنها. ويُعد IPython مناسبًا بقوة لأنه يمنح النموذج مساحة عمل حسابية مستمرة:
...
يغطي كل جزء نمطًا مختلفًا من أنماط الفشل:
...
ومعًا، تتيح هذه المكونات للوكيل الانتقال بين الاستدلال اللغوي والحساب الحتمي دون تحويل كل قيمة وسيطة إلى نص داخل المطالبة.
...
القاعدة الحاكمة بسيطة عمدًا:

> **Python هي لغة التنسيق؛ ويظل المضيف حدّ السلطة.**
...
تم التحقق في المرشح المجمّد من:

- تغطية Python من 3.11 إلى 3.14؛
- سطح MCP v7 ذي 30 أداة؛
- مخطط SQLite تزايدي حتى v4؛
- تشغيل المستودع بالكامل: **199 passed, 1 platform-gated skip**؛
- اختبارات exact-wheel نظيفة على Linux/WSL وعلى Windows الأصلي؛
...
- الاحتفاظ بأدلة الانتقال المثبت لـ Hermes في WAL المشروع ذي الإلحاق فقط.
