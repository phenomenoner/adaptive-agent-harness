...
| S0 | SDD successor for activation/provider-ready authority domains | doing | P1, A0 | contract prerequisites; no implementation row closure | Freeze fourteen complete schemas, acyclic prepared/terminal marker authority, exhaustive v6 cell-free planner outcomes, append-only activation history, complete grant_ids resolver, bounded paired-planning schema with no T5 launch authority, mandatory admission gates and incident-shaped assertions; regenerate receipt; deterministic dual review/freeze. |
| A1a | Activation/adapter strict models | blocked | S0 | prerequisite slice; no row closure alone | New transport-neutral typed models and direct RED→GREEN tests for §§1–2; no registry/generator/package edits. |
| A1b | Operator/readback/evaluation contract models | blocked | A1a | contract prerequisites for A-MIG-008/A-MIG-009/A-MIG-010 and later B/C/E/G rows | Activation-generation authority, cutover/restore prepared/receipt/terminal/readback, server grant-set, provider-alias, evaluator-classification and paired-planning strict models with direct tests; no launcher/attempt authority. |
| A1c | Contract registry/generators/fixtures | blocked | A1b | A-ACT-001, A-SEC-001 | Canonical model registry, duplicate-object-key-rejecting byte loader before JSON/model validation, checked-in schemas, valid/invalid fixtures, dual-validator and secret/tamper matrix. |
