...
| S0 | SDD successor for activation/provider-ready authority domains | verified | P1, A0 | contract prerequisites; no implementation row closure | Generation-10 independent `PASS / BATCH_COMPLETE`, `findings=[]`; exact reviewed SDD/receipt identities frozen. |
| A1a | Activation/adapter strict models | planned | S0 | prerequisite slice; no row closure alone | Rev2 packet independent `PASS / BATCH_COMPLETE`; task-start parent/collision gate remains before dispatch. New transport-neutral typed models and direct RED→GREEN tests for §§1–2; no registry/generator/package edits. |
