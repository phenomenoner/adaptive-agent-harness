...
- `caller_work_command_receipts`: append-only replay storage for caller mutation commands. `(operation_id, idempotency_key)` binds one command kind/digest to an exact result ticket JSON, digest and revision. SQLite triggers forbid update/delete; checks bind valid JSON to the same operation, ticket and revision and constrain digest shape. The sole write owner is the caller-work repository transaction: before any ticket or external mutation it validates the strict command contract, canonicalizes and digests the complete command, and looks up this key. An existing equal digest returns the stored strict `CallerWorkTicket`; an existing unequal digest or cross-command/cross-ticket key reuse conflicts. For a new key, ticket CAS, result canonicalization/digest validation and receipt insert commit atomically. T2—not this migration receipt—must prove all five v8 mutation replay paths, changed-payload no-mutation behavior and no extra dispatch.
