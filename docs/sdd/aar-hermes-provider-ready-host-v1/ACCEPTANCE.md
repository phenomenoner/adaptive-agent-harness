...
| AC-11 / #2 | An external operator can derive the exact `aar.install-candidate-receipt.v1` from the released wheel and source identity without private tooling; wheel/member tamper or source mismatch fails closed. | T1/T2 | public CLI positive fixture; one-axis wheel/member/source negatives; released wheel readback produces byte-identical published candidate receipt. |
...
| AC-18 | PR head equals reviewed candidate, remote CI is green, merge/tag/prerelease/assets bind the exact released bytes; issues #2/#3/#4 close traceably. | Release | PR readback, check runs, merge SHA, annotated tag, GitHub prerelease, wheel/sdist/candidate-receipt/release-receipt byte readback, issue state/comments. |
