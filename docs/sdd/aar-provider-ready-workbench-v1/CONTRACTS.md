...
The package-owned mapping is immutable and method-scoped. In frozen method order the factory IDs are `aar.factory.model-request.v1`, `aar.factory.subagent-submit.v1`, `aar.factory.subagent-result.v1`, `aar.factory.evidence-query.v1`, `aar.factory.artifact-put.v1`, and `aar.factory.effect-propose.v1`. All six declarations bind the existing owner member `aar/runtime/brokers.py`; sharing one member does not merge factory identities. Each receipt entry independently carries that member's exact candidate-wheel SHA-256, and every adapter's method, factory ID, contract ID, request schema digest, and response schema digest must equal the corresponding record in `aar-broker-catalog-v2.json`. Route and grant-policy values remain explicit operator inputs and are not package defaults.
